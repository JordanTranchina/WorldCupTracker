...
import { getAllTeams, FIFA_RANKINGS, type GroupTeam } from '@/lib/groups';

const FEATURED_TEAMS = [
  { code: 'all', label: 'All Teams', flag: '🌎' },
  { code: 'favorites', label: 'Favorites', flag: '⭐' },
...
export default function ScheduleView({ matches }: { matches: Match[] }) {
  ...
  const FAVORITES = ['usa', 'eng', 'esp', 'bra'];

  const filtered = matches.filter((m) => {
    if (activeTeam === 'all') return true;
    if (activeTeam === 'favorites') {
      return FAVORITES.includes(m.homeCode) || FAVORITES.includes(m.awayCode);
    }
    return m.homeCode === activeTeam || m.awayCode === activeTeam;
  });
  ...
  // Check if current filter is a featured team or "other"
  const isFeatured = FEATURED_TEAMS.some(t => t.code === activeTeam);
        ...
        <div className="max-w-2xl mx-auto px-4 py-4">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div>
              ...
              <p className="text-slate-400 text-xs">Group Stage Schedule</p>
            </div>
          ...
          <div className="mt-2">
            <select
              value={isFeatured ? '' : activeTeam}
              onChange={(e) => {
                if (e.target.value) setActiveTeam(e.target.value);
              }}
              className="w-full px-3 py-2 rounded-xl text-sm bg-slate-800 text-slate-300 border border-slate-700 focus:border-emerald-500 focus:outline-none transition-colors appearance-none"
            ...
            >
              <option value="">🔍 Filter by any team...</option>
              {allTeams.map((t) => (
                <option key={t.code} value={t.code}>
                  {t.flag} {t.name} ({FIFA_RANKINGS[t.code] || '—'})
                </option>
              ))}
            </select>
          </div>
